...
log "Preparing host tools required for ISO generation"
if command -v apt-get >/dev/null 2>&1; then
  apt-get update -y >/dev/null
  apt-get install -y -q xorriso grub-pc-bin grub-efi-amd64-bin mtools squashfs-tools rsync >/dev/null
else
  die "Host package manager apt-get not found; install xorriso, grub-mkrescue, and mksquashfs manually"
fi

if ! command -v grub-mkrescue >/dev/null 2>&1; then
  die "grub-mkrescue not found after host tool install"
fi
if ! command -v mksquashfs >/dev/null 2>&1; then
  die "mksquashfs not found after host tool install"
fi

log "Ensuring chroot has dracut for live initramfs generation"
run_in_chroot "
set -euo pipefail
set +u
source /etc/profile
...
if [[ ! -f "$INITRAMFS_IMAGE" ]]; then
  log "Generating live initramfs for ${KERNEL_VERSION}"
  run_in_chroot "
set -euo pipefail
set +u
source /etc/profile
...
log "Building compressed live rootfs"
mksquashfs "$TARGET_ROOT" "$ISO_ROOT/LiveOS/rootfs.squashfs" \
  -wildcards \
  -e proc sys dev run tmp var/tmp var/cache/distfiles
...
menuentry "GentooHA Live (systemd + Supervisor host)" {
  linux /boot/vmlinuz root=live:CDLABEL=${ISO_LABEL} rd.live.image rw console=tty0
  initrd /boot/initramfs.img
}
EOF

OUT_ISO="$ARTIFACT_DIR/$ISO_NAME"
log "Creating ISO at $OUT_ISO"
grub-mkrescue -o "$OUT_ISO" "$ISO_ROOT" -- -V "$ISO_LABEL" >/dev/null
